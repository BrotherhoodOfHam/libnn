...
void gan::train(const std::vector<vector>& data, size_t epochs)
{
	vector z_input(_g->input_size());
	vector g_value(28 * 28);
	vector y_d_1(1); y_d_1[0] = 0.9f;
	vector y_d_0(1); y_d_0[0] = 0;
	vector y_g(1); y_g[0] = 1;
	...
	for (size_t e = 0; e < epochs; e++)
	{
		...
		for (const auto& x_data : data)
		{
			...
			//randomize z
			for (size_t i_z = 0; i_z < z_input.length; i_z++)
				z_input[i_z] = (scalar)unif(rng);

			_d->train_batch(x_data, y_d_1, k, r);
			_d->train_batch(_g->forward(z_input), y_d_0, k, r);
